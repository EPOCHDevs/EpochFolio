...
TEST_CASE("Tearsheet") {
  ...
  auto factor_returns_path = ("s3://epoch-db/DailyBars/Stocks/SPY.parquet.gzip");
  ...
  auto test_returns = epoch_frame::read_csv_file(test_returns_path, {
    .has_header = false
  }).set_index("f0").to_series();
  auto index = test_returns.index();

  test_returns = test_returns.set_index(index->Make(index->array().cast(utc).value()));

  auto test_factor = epoch_frame::read_parquet(factor_returns_path).set_index("t")["c"].pct_change().loc({
    test_returns.index()->at(0),
     test_returns.index()->at(-1)
  });
  test_factor = test_factor.set_index(index->Make(test_factor.index()->array().cast(utc).value()));
  ...
  test_pos = test_pos.set_index(index->Make(test_pos.index()->array().cast(utc).value()));
  ...
  round_trip = round_trip.assign("openDateTime", round_trip["open_dt"].cast(utc)).drop("open_dt");
  round_trip = round_trip.assign("closeDateTime", round_trip["close_dt"].cast(utc)).drop("close_dt");
  ...
  round_trip = round_trip.assign("side", round_trip["long"].map([]( auto const& v) {
    return epoch_frame::Scalar{v.as_bool() ? std::string{"Long"} : "Short"};
  }));
  round_trip = round_trip.drop("long");

  auto test_result =  PortfolioTearSheetFactory{TearSheetDataOption{
    test_returns,
    test_factor,
    cash,
    test_pos,
    ...
    round_trip,
    sector,
  false}}.MakeTearSheet(TearSheetOption{});

  write_json(test_result, "full_test_result.json");
  write_json(test_result.positions, "positions_test_result.json");
  write_json(test_result.strategy_benchmark, "strategy_benchmark_test_result.json");
  ...
  write_json(test_result.returns_distribution, "returns_distribution_test_result.json");
