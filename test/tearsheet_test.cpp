...
TEST_CASE("Tearsheet without benchmark") {
  using namespace epoch_folio;
  ...
  // Create tearsheet without benchmark (passing std::nullopt)
  auto test_result = PortfolioTearSheetFactory{
      TearSheetDataOption{
          test_returns, std::nullopt, cash, test_pos, test_txn, round_trip,
          sector, false}}.MakeTearSheet(TearSheetOption{});

  // Create output directory if it doesn't exist
  std::string output_dir = "test_output_no_benchmark";
  std::filesystem::create_directories(output_dir);

  write_protobuf(test_result, output_dir + "/full_test_result.pb");
  
  // Verify that tearsheet was created successfully without benchmark
  auto &categories = test_result.categories();
  REQUIRE(categories.contains(epoch_folio::categories::Positions));
  REQUIRE(categories.contains(epoch_folio::categories::StrategyBenchmark));
  REQUIRE(categories.contains(epoch_folio::categories::Transactions));
}