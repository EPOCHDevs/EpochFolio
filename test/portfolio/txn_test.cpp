...
TEST_CASE("Transaction") {
    auto dates = date_range({.start="2015-01-01"_date, .periods=20, .offset=factory::offset::days(1)});
    auto positions = make_dataframe(dates,
                            std::vector{
                                        std::vector<double>{40, 10, 40, 10, 40, 10, 40, 10, 40, 10,
                                           40, 10, 40, 10, 40, 10, 40, 10, 40, 10},
                            std::vector(dates->size(), 10.0)},
                            std::vector<std::string>{"0", "cash"});

    SECTION("No Transactions") {
        auto transactions = make_dataframe(dates, std::vector<std::vector<Scalar>>{},
                                           {arrow::field("sid", arrow::int32()),
                                            arrow::field("amount", arrow::float64()),
                                            arrow::field("price", arrow::float64()),
                                            arrow::field("symbol", arrow::utf8())}).reset_index("timestamp");
    ...
    SECTION("Multiple Transactions") {
        ...
        auto transactions1 = make_dataframe(index, std::vector{
            ...
            factory::array::make_array(std::vector<std::string>(dates->size(), "0"))
        }, {"timestamp", "sid", "amount", "price", "symbol"});

        auto transactions2 = make_dataframe(index, std::vector{
    dates->array().as_chunked_array(),
    ...
    factory::array::make_array(std::vector<std::string>(dates->size(), "0"))
        ...
        auto transactions = concat({.frames={transactions1, transactions2}, .ignore_index = true});
