...
TEST_CASE("Drawdown Test") {
  auto make_index = [](int64_t size) {
    return date_range({.start = "2000-01-03"_date,
                       .periods = size,
  ...
  SECTION("Test Max Drawdown Relative") {
    std::vector<double> drawdownList = {100, 110, 120, 150,  180, 200, 100,
                                        120, 160, 180, 200,  300, 400, 500,
                                        600, 800, 900, 1000, 650, 600};
    auto drawdownSeries =
        make_series(make_index(20), drawdownList, "") / 10.0_scalar;
  ...
  SECTION("Test Get Max Drawdown") {
    ...
    std::vector<TestCase> cases = {{{100, 120, 100, 80, 70, 110, 180, 150},
                                    Date(2000y, January, 4d),
                                    Date(2000y, January, 7d),
                                    Date(2000y, January, 9d)},
                                   {{100, 120, 100, 80, 70, 80, 90, 90},
                                    Date(2000y, January, 4d),
                                    Date(2000y, January, 7d),
                                    std::nullopt}};
    ...
    for (const auto &[i, testCase] : std::views::enumerate(cases)) {
      DYNAMIC_SECTION("Drawdown empyrical " << i) {
        auto dt = date_range({.start = "2000-01-03"_date,
                              .periods = testCase.prices.size(),
                              .offset = offset::days(1)});
  ...
  SECTION("Test Gen Drawdown table") {
    ...
    std::vector<TestCase> cases = {
        {px_list_2, Date(2000y, January, 4d), Date(2000y, January, 7d),
         std::nullopt, std::nullopt},
        {px_list_1, Date(2000y, January, 4d), Date(2000y, January, 7d),
         Date(2000y, January, 9d), 4}};
  ...
  SECTION("Test Overlap") {
    ...
    auto spy_returns = make_series(date_range({.start = "2005-01-02"_date,
                                               .periods = n_samples,
                                               .offset = offset::days(1)}),
                                   returns);
    ...
    REQUIRE(drawdowns.size() >
            1); // Ensure we have at least 2 drawdowns to compare
...
TEST_CASE("Gross Leverage Test") {
  ...
  SliceType slice{Scalar{"2004-02-01"__date}.cast(
                      arrow::timestamp(arrow::TimeUnit::NANO, "UTC")),
                  Scalar{}};

  auto test_gross_lev =
      epoch_frame::read_csv_file(get_s3_test_path("test_gross_lev.csv"),
                                 {
                                     .has_header = false,
                                 })
          .ValueOrDie()
  ...
  INFO(result << "\n!=\n" << test_gross_lev);
  REQUIRE(result.is_approx_equal(test_gross_lev,
...
TEST_CASE("Stats") {
  ...
  auto simple_rets = make_series(date_range({.start = "2000-01-03"_date,
                                             .periods = 500,
                                             .offset = offset::days(1)}),
                                 simple_rets_data);
  ...
  auto simple_benchmark = make_series(date_range({.start = "2000-01-01"_date,
                                                  .periods = 500,
                                                  .offset = offset::days(1)}),
  ...
  auto dt = date_range(
      {.start = "2000-01-03"_date, .periods = 3, .offset = offset::days(1)});

  std::vector<double> px_list_2 = {1.0, 1.2, 1.0, 0.8, 0.7, 0.8, 0.8, 0.8};
  auto dt_2 = date_range(
      {.start = "2000-01-03"_date, .periods = 8, .offset = offset::days(1)});
  ...
  SECTION("Test Rolling Sharpe") {
    ...
    // Expected values from Python: [np.nan, np.inf, np.inf, 11.224972160321,
    // np.nan]
    auto rolling_sharpe_result = RollingSharpe(returns, rolling_sharpe_window);
    rolling_sharpe_result = rolling_sharpe_result.where(
        !Array{arrow::compute::IsNan(rolling_sharpe_result.array())
                   ->chunked_array()
                   ->chunk(0)},
        Scalar{});
    auto expected_result = factory::array::make_array(std::vector<double>{
        std::numeric_limits<double>::quiet_NaN(),
        std::numeric_limits<double>::infinity(),
        std::numeric_limits<double>::infinity(), 11.224972160321,
        std::numeric_limits<double>::quiet_NaN()});

    INFO(rolling_sharpe_result << "\n!=\n" << expected_result->ToString());
    REQUIRE(rolling_sharpe_result.array()->ApproxEquals(
        *expected_result, arrow::EqualOptions{}.nans_equal(true)));
  }
  ...
  //     auto rolling_beta_result = RollingBeta(concat({.frames={returns,
  //     benchmark_rets}, .axis=AxisType::Column}), rolling_window);
  //
  //     // Check specific value at index 2
  //     INFO(rolling_beta_result);
  //     REQUIRE(rolling_beta_result.iloc(2).value()->ApproxEquals(*arrow::MakeScalar(0)));
