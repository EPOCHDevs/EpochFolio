...
namespace epoch_folio::round_trip {
    std::vector<double> linspace(double start, double end, int64_t num, bool endPoint=true) {
    ...
    TearSheetFactory::TearSheetFactory(epoch_frame::DataFrame round_trip, epoch_frame::Series returns, epoch_frame::DataFrame positions, SectorMapping sector_mapping):
    m_round_trip(std::move(round_trip)),
    m_returns(std::move(returns)),
    m_positions(std::move(positions)),
    m_sector_mapping(std::move(sector_mapping))
    {}

    XRangeDef TearSheetFactory::MakeXRangeDef(epoch_frame::DataFrame const &trades) const {
        // round_trip_lifetimes
        auto symbol_series = trades["symbol"];
        XRangeDef xrange;
        xrange.chartDef = ChartDef{"xrange", "Round trip lifetimes", EpochFolioDashboardWidget::XRange, EpochFolioCategory::RoundTrip, AxisDef{"Asset", "Asset"}};
        xrange.categories = Array{symbol_series.unique()}.to_vector<std::string>();
        xrange.points.resize(trades.num_rows());

        auto date_range = trades[std::vector<std::string>{"open_dt", "close_dt", "long"}];
        tbb::parallel_for(tbb::blocked_range<size_t>(0,  xrange.categories.size()), [&](auto const &range) {
            for (auto i = range.begin(); i != range.end(); ++i) {
                auto symbol = Scalar{ xrange.categories[i]};
                auto trades_in_sector = date_range.loc(symbol_series == symbol);
                for (int64_t j = 0; j < static_cast<int64_t>(trades_in_sector.num_rows()); ++j) {
                    ...
                    xrange.points[index.value<uint64_t>().value()] = XRangePoint{
                        open_dt,
                        close_dt,
                        i,
                        long_.as_bool()
    ...
    LinesDef TearSheetFactory::MakeProbProfitChart(epoch_frame::DataFrame const &trades) const {
        LinesDef prob_profit_chart{
            .chartDef = ChartDef{"prob_profit_trade", "Probability of making a profitable decision", EpochFolioDashboardWidget::Lines, EpochFolioCategory::RoundTrip,
        ...
        std::transform(x.begin(), x.end(), y.begin(), [&](double x_) {
            return pdf(dist, x_);
        });

        auto points = std::views::iota(0, kMaxPoints) | epoch_core::ranges::to_vector_v;
        prob_profit_chart.lines.emplace_back(MakeSeriesLine(points, y));

        prob_profit_chart.straightLines.emplace_back("2.5%", Scalar{quantile(dist, 0.025)}, true);
        prob_profit_chart.straightLines.emplace_back("97.5%", Scalar{quantile(dist, 0.975)}, true);

        return prob_profit_chart;
    }

    HistogramDef TearSheetFactory::MakeHoldingTimeChart(epoch_frame::DataFrame const &trades) const {
        HistogramDef holding_time_chart{
            .chartDef = ChartDef{"holding_time", "Holding time in days", EpochFolioDashboardWidget::Histogram, EpochFolioCategory::RoundTrip},
            .data = (trades["duration"] / (86400_scalar * 1e9_scalar)).floor().cast(arrow::int64()).contiguous_array()
        };
        return holding_time_chart;
    }

    HistogramDef TearSheetFactory::MakePnlPerRoundTripDollarsChart(epoch_frame::DataFrame const &trades) const {
        HistogramDef pnl_per_round_trip_dollars_chart{
            .chartDef = ChartDef{"", "PnL per round trip in dollars", EpochFolioDashboardWidget::Histogram, EpochFolioCategory::RoundTrip},
            .data = trades["pnl"].contiguous_array()
        };
        return pnl_per_round_trip_dollars_chart;
    }

    HistogramDef TearSheetFactory::MakeReturnsPerRoundTripDollarsChart(epoch_frame::DataFrame const &trades) const {
        HistogramDef returns_per_round_trip_dollars_chart{
            .chartDef = ChartDef{"", "Returns per round trip in dollars", EpochFolioDashboardWidget::Histogram, EpochFolioCategory::RoundTrip},
    ...
    void TearSheetFactory::Make(FullTearSheet& output) const {
        ...
        auto returns_per_round_trip_dollars = MakeReturnsPerRoundTripDollarsChart(trades);
        
        output.round_trip = TearSheet{
            .cards = {},
            .charts = std::vector<Chart>{pie_chart, xrange, prob_profit_trade, holding_time, pnl_per_round_trip_dollars, returns_per_round_trip_dollars},
            .tables = tables
        };
    }
    ...
    DataFrame TearSheetFactory::ExtractRoundTrips() const {
        ...
        Series open_dt = m_round_trip["openDateTime"];
        auto close_dt = m_round_trip["closeDateTime"];
        auto is_long = m_round_trip["side"] == "Long"_scalar;
        auto symbol = m_round_trip["asset"];
        auto pnl = m_round_trip["netReturn"];
        auto duration = open_dt.dt().nanoseconds_between(close_dt.contiguous_array());

        auto portfolio_value = m_positions.sum(AxisType::Column) / (1.0_scalar + m_returns);
        auto pv_date = portfolio_value.index()->as_chunked_array();
        auto pv_table = arrow::Table::Make(arrow::schema({
        ...
        auto round_trip_table = arrow::Table::Make(arrow::schema({
        ...
        }), {open_dt.array(), close_dt.array(), is_long.array(), symbol.array(), std::make_shared<arrow::ChunkedArray>(duration.value()), pnl.array(), rt_date});

        ac::Declaration left{"table_source", ac::TableSourceNodeOptions(round_trip_table)};
        ...
        auto returns = AssertArrayResultIsOk(arrow::compute::Divide(tmp->GetColumnByName("pnl"), tmp->GetColumnByName("portfolio_value")));
        tmp = AssertTableResultIsOk(tmp->AddColumn(tmp->num_columns(), arrow::field("returns", returns->type()), returns));
        tmp = AssertTableResultIsOk(tmp->RemoveColumn(tmp->schema()->GetFieldIndex("date")));

        return make_dataframe(tmp);
    }

    PieDef TearSheetFactory::MakeProfitabilityPieChart(epoch_frame::DataFrame const &trades) const {
        DataFrame profit_attribution = GetProfitAttribution(trades);

        auto sectors = profit_attribution.index()->array().map([this](Scalar const &symbol) {
            return Scalar{lookupDefault(m_sector_mapping, symbol.repr(), std::string{"Others"})};
        });
        auto sector_profit_attr = profit_attribution.group_by_agg(sectors.as_chunked_array()).sum().to_series();
        ...
        return PieDef{
            ChartDef{"profitability_pie", "Profitability (PnL / PnL total)", EpochFolioDashboardWidget::Pie, EpochFolioCategory::RoundTrip},
