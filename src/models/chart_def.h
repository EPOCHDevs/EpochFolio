...
namespace epoch_folio {
    ...
    struct BoxPlotDataPoint {
        ...
        static std::pair<BoxPlotDataPoint, BoxPlotOutliers> Make(
    ...
    using Chart = std::variant<LinesDef, HeatMapDef, BarDef, HistogramDef, BoxPlotDef, XRangeDef, PieDef>;

    template<typename X, typename Y>
    Line MakeSeriesLine(std::vector<X> x, std::vector<Y> y, std::optional<std::string> const& name=std::nullopt) {
        Line line;
        line.data = std::vector<Point>(x.size());
        for (size_t i = 0; i < x.size(); ++i) {
            line.data[i] = Point{epoch_frame::Scalar{std::move(x[i])}, epoch_frame::Scalar{std::move(y[i])}};
        }
    ...
    Line MakeSeriesLine(const epoch_frame::Series& series, std::optional<std::string> const& name=std::nullopt);
    SeriesLines MakeSeriesLines(const epoch_frame::Series& seriesA, const epoch_frame::Series& seriesB, std::optional<std::string> const& nameA=std::nullopt, std::optional<std::string> const& nameB=std::nullopt);
}
