...
namespace epoch_folio
{
    ...
    Table GetSymbolsTable(epoch_frame::DataFrame const &round_trip, AggList const &stats_dict) {
        auto apply_symbol = [&](std::string const& symbol, epoch_frame::Series const &returns)
        {
            std::vector<std::string> index(stats_dict.size());
            std::vector<Scalar> all_trades(stats_dict.size());

            std::transform(stats_dict.begin(), stats_dict.end(), index.begin(), all_trades.begin(), [&](auto const &stat, std::string &row)
            {
                row = stat.first;
                return std::visit([&]<typename T>(T const& fn) {
                 if constexpr (std::is_same_v<T, std::string>) {
            ...
            return make_dataframe(factory::index::make_object_index(index), {all_trades}, {arrow::field(symbol, arrow::float64())});
        };

        auto groups = round_trip[std::vector<std::string>{"returns", "symbol"}].group_by_apply("symbol").groups();
        std::vector<FrameOrSeries> frames;
        ColumnDefs column_defs{
            {
                "key", "Stats", epoch_core::EpochFolioType::String
            }};
        ...
        for (auto const& [symbol, indexes] : groups) {
            ...
            frames.emplace_back(apply_symbol(symbol_name, returns.iloc(Array{indexes})));
            column_defs.emplace_back(symbol_name, symbol_name, epoch_core::EpochFolioType::Percent);
        }

        auto table = concat({.frames = frames, .axis = AxisType::Column}).reset_index("key").table();
        return Table{epoch_core::EpochFolioDashboardWidget::DataTable, epoch_core::EpochFolioCategory::RoundTrip, column_defs, table};
    }
    ...
    std::vector<Table> GetRoundTripStats(epoch_frame::DataFrame const &round_trip) {
        ...
        static const AggList PNL_STATS{
                {"Total profit", "sum"},
                {"Gross profit", [](Series const &x)
                { return x.loc(x > ZERO).sum(); }},
               {"Gross loss", [](Series const &x)
               { return x.loc(x < ZERO).sum(); }},
              {"Profit factor",
               [](Series const &x)
               {
              ...
              {"Avg. winning trade", [](Series const &x)
              { return x.loc(x > ZERO).mean(); }},
             {"Avg. losing trade", [](Series const &x)
             { return x.loc(x < ZERO).mean(); }},
        ...
        static const AggList SUMMARY_STATS{
                {"Total number of round_trips", [](Series const &x) {
                    return x.count_valid();
                }},
                {"Percent profitable", [](Series const &x)
                { return Scalar{static_cast<double>(x.loc(x > ZERO).size()) / static_cast<double>(x.size())}; }},
               {"Winning round_trips", [](Series const &x)
               { return Scalar{x.loc(x > ZERO).size()}; }},
              {"Losing round_trips", [](Series const &x)
              { return Scalar{x.loc(x < ZERO).size()}; }},
             {"Even round_trips", [](Series const &x)
             { return Scalar{x.loc(x == ZERO).size()}; }},
        };

        static const AggList RETURNS_STATS{
                {"Avg returns all round_trips", "mean"},
                {"Avg returns winning", [](Series const &x)
                { return x.loc(x > ZERO).mean(); }},
               {"Avg returns losing", [](Series const &x)
               { return x.loc(x < ZERO).mean(); }},
              {"Median returns all round_trips", [](Series const &x) {
                  return x.quantile(arrow::compute::QuantileOptions{0.5, arrow::compute::QuantileOptions::Interpolation::LINEAR});
              }},
              {"Median returns winning", [](Series const &x)
              { return x.loc(x > ZERO).quantile(arrow::compute::QuantileOptions{0.5, arrow::compute::QuantileOptions::Interpolation::LINEAR}); }},
             {"Median returns losing", [](Series const &x)
             { return x.loc(x < ZERO).quantile(arrow::compute::QuantileOptions{0.5, arrow::compute::QuantileOptions::Interpolation::LINEAR}); }},
            {"Largest winning trade", "max"},
            {"Largest losing trade", "min"},
        };
        ...
        static const AggList DURATION_STATS{
                ...
                // A potential solution would be to modify AggAllLongShort to accept multiple
                // fields or pre-calculate these metrics before passing to AggAllLongShort.
                //
                // {"Avg # round_trips per day", [](DataFrame const& rt) {
                //     auto trading_period_days = (rt["closeDateTime"].dt().max() - rt["openDateTime"].dt().min()).days();
                //     return static_cast<double>(rt.num_rows()) / trading_period_days;
                // }},
                // {"Avg # round_trips per month", [](DataFrame const& rt) {
                //     auto trading_period_days = (rt["closeDateTime"].dt().max() - rt["openDateTime"].dt().min()).days();
                //     return static_cast<double>(rt.num_rows()) / (trading_period_days / 21.0); // ~21 trading days per month
                // }}
            };
        ...
        return {Table{epoch_core::EpochFolioDashboardWidget::DataTable, epoch_core::EpochFolioCategory::RoundTrip,         ColumnDefs{
                    {"key", "PnL Stats", epoch_core::EpochFolioType::String},
                    {"all_trades", "All Trades", epoch_core::EpochFolioType::Monetary},
                    {"long_trades", "Long Trades", epoch_core::EpochFolioType::Monetary},
                    {"short_trades", "Short Trades", epoch_core::EpochFolioType::Monetary},
                }, pnl},
                Table{epoch_core::EpochFolioDashboardWidget::DataTable, epoch_core::EpochFolioCategory::RoundTrip,         ColumnDefs{
                    {"key", "Summary Stats", epoch_core::EpochFolioType::String},
                    {"all_trades", "All Trades", epoch_core::EpochFolioType::Decimal},
                    {"long_trades", "Long Trades", epoch_core::EpochFolioType::Decimal},
                    {"short_trades", "Short Trades", epoch_core::EpochFolioType::Decimal},
                }, summary},
                Table{epoch_core::EpochFolioDashboardWidget::DataTable, epoch_core::EpochFolioCategory::RoundTrip,         ColumnDefs{
                    {"key", "Duration Stats", epoch_core::EpochFolioType::String},
                    {"all_trades", "All Trades", epoch_core::EpochFolioType::Duration},
                    {"long_trades", "Long Trades", epoch_core::EpochFolioType::Duration},
                    {"short_trades", "Short Trades", epoch_core::EpochFolioType::Duration},
                }, duration},
                Table{epoch_core::EpochFolioDashboardWidget::DataTable, epoch_core::EpochFolioCategory::RoundTrip,         ColumnDefs{
                    {"key", "Return Stats", epoch_core::EpochFolioType::String},
                    {"all_trades", "All Trades", epoch_core::EpochFolioType::Percent},
                    {"long_trades", "Long Trades", epoch_core::EpochFolioType::Percent},
                    {"short_trades", "Short Trades", epoch_core::EpochFolioType::Percent},
                }, returns},
                    GetSymbolsTable(round_trip, RETURNS_STATS)};
    }

    DataFrame GetProfitAttribution(epoch_frame::DataFrame const &round_trip, std::string const &col)
    {
        const auto total_pnl = round_trip["pnl"].sum();
        return  (round_trip[std::vector<std::string>{"pnl", col}].group_by_agg(col).sum() / total_pnl);
    }
