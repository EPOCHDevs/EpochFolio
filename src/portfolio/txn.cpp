...
namespace epoch_folio {
    epoch_frame::Series GetTurnover(epoch_frame::DataFrame const &positions,
        ...
        epoch_frame::Series tradedValue = GetTransactionVolume(transactions)["txn_volume"];
    ...
    epoch_frame::DataFrame GetTransactionVolume(epoch_frame::DataFrame const &df) {
        ...
        epoch_frame::Series values = df.contains("txn_volume") ? df["txn_volume"] : amounts * df["price"];

        auto table = epoch_frame::concat(
            {.frames = {
                epoch_frame::FrameOrSeries{amounts.to_frame("txn_shares")},
                epoch_frame::FrameOrSeries{values.to_frame("txn_volume") } },
                .axis = epoch_frame::AxisType::Column});

        return table.group_by_agg(df["timestamp"].dt().normalize().as_chunked_array()).sum();
    }
}